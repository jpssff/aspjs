...
//Init Tasks (Sync)
var m, dispatch_data = fs.readFileSync(path.join(base_path, 'app/routes.js'), 'utf8');
...
function buildControllerStubs(map) {
  ...
  fs.readdirSync(p).forEach(function(n) {
    var file = path.join(__dirname, '../build', n);
...
function buildControllerStub(name, controllers, cfg) {
  ...
  ['system', 'shared', 'models'].forEach(function(n) {
    ...
    var list = [], contents = fs.readFileSync(path.join(base_path, 'app', n, '_inc.xml'), 'utf8');
    contents.replace(/<script.*?src="([^"]*)".*?><\/script>/g, function(tag, p) {
      tag = tag.replace(' runat="server"', '');
      tag = tag.replace(p, p.replace(/\//g, '\\'));
      tag = tag.replace(/adapters\\(.*?)\.js/, 'adapters\\wsh.js');
  ...
  if (cfg && cfg[name] && cfg[name].inc) {
    cfg[name].inc.split(/[,\s]+/).forEach(function(script) {
      scripts.push('<script language="javascript" src="..\\..\\app\\' + script.replace(/\//g, '\\') + '"><\/script>');
    });
  }
  var output = [];
  ...
  output.push('</job>');
  output.push('</package>');
  fs.writeFileSync(path.join(__dirname, '../build', name + '.wsf'), output.join('\r\n'));
}
...
function processRequest(engine, req, res) {
  ...
  function writeHead(res_data) {
    ...
    var status = res_data.status.match(/^(\d+)(\s+.*)?$/);
    if (status) {
...
function proxy(engine, req_data, callback, errback) {
  ...
  if (engine == 'wsh') {
    script_path = path.join('build', script + '.wsf').replace(/\//g, '\\');
