<%@LANGUAGE="JAVASCRIPT" CODEPAGE="65001"%>
<script runat="server" language="javascript" src="config/config.js"></script>
<script runat="server" language="javascript" src="system/config/config.js"></script>
<!--#include file="system/_inc.asp" -->
<!--#include file="code/_inc.asp" -->
<script runat="server" language="javascript" src="system/lib/activerecord.js"></script>
<script runat="server" language="javascript" src="controllers/test.js"></script>
<script runat="server" language="javascript">
/*!
 * IMPORTANT: Ensure the script tag above points to corresponding controller file.
 *
 * Controller Stub
 *
 * This stub handles the request after dispatch.asp has finished. It just includes the appropriate
 * files (above) and calls the app_init function to begin request processing.
 *
 */
app_init();
</script>
