...
function lib_sizzle() {
  ...
  // Check if JS engine is using optimization where it does not always call our comparison function.
  [0, 0].sort(function() {
  ...
  var Expr = Sizzle.selectors = {
    ...
    preFilter: {
      CLASS: function(match, curLoop, inplace, result, not) {
        ...
        for (var i = 0, elem; (elem = curLoop[i]) != null; i++) {
          if (elem) {
            if (not ^ (elem.getAttribute('class') && (' ' + elem.getAttribute('class') + ' ').replace(/[\t\n\r]/g, ' ').indexOf(match) >= 0)) {
              if (!inplace) {
                result.push(elem);
              }
    ...
    filters: {
      ...
      radio: function(elem) {
        return elem.nodeName().toLowerCase() == 'input' && elem.getAttribute('type') == 'radio';
      },

      checkbox: function(elem) {
        return elem.nodeName().toLowerCase() == 'input' && elem.getAttribute('type') == 'checkbox';
      },

      file: function(elem) {
        return elem.nodeName().toLowerCase() == 'input' && elem.getAttribute('type') == 'file';
      },

      password: function(elem) {
        return elem.nodeName().toLowerCase() == 'input' && elem.getAttribute('type') == 'password';
      },

      submit: function(elem) {
        var name = elem.nodeName().toLowerCase();
        return (name == 'input' || name == 'button') && elem.getAttribute('type') == 'submit';
      },

      image: function(elem) {
        return elem.nodeName().toLowerCase() == 'input' && elem.getAttribute('type') == 'image';
      },

      reset: function(elem) {
        var name = elem.nodeName().toLowerCase();
        return (name == 'input' || name == 'button') && elem.getAttribute('type') == 'reset';
      },

      button: function(elem) {
        var name = elem.nodeName().toLowerCase();
        return name == 'input' && elem.getAttribute('type') == 'button' || name == 'button';
      },
  ...
  function sortOrder(a, b) {
    var aup = a.parentNode(), bup = b.parentNode();
    ...
    // Otherwise we need to build a full list of the parentNodes for comparison
    var ap = [], bp = [], cur;
    cur = aup;
    while (cur) {
      ap.unshift(cur);
